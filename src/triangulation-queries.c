...
//todo: rewise the mpicomm wrapper, as Alberto suggested
...
int p4estw_get_quadrants2(p4est_wrapper_t *p4estw, int *tree_indices, int *tree_offsets, int *quadrant_data, int *quadsxlevel)
{
       ...
       tt <= p4estw->p4est->last_local_tree; ++tt, ++local_tree_num)
  {
    ...
    for (q = 0; q < Q; ++q, ++k) {
      ...
      quadrant_data[DATA_PER_QUAD * k + 2] = quad->level;
      elem_data = (element_data_t* ) quad->p.user_data;
      elem_data->local_idx = k;   // enumerate from 0, in p4est. In Fempar, from 1.
      quadsxlevel[quad->level] = quadsxlevel[quad->level] + 1;     
    }
  }
  tree_offsets[local_tree_num] = k + 1;  // enumerate from 1 for Fortran
